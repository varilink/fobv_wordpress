...
function fobv_subscribe() {
    ...
    if ( function_exists( 'vl_recaptcha_verify_user_response') ) {
        ...
        $remoteip = $recaptcha_verification_result['remoteip'];
        $rc = $recaptcha_verification_result['rc'];
        $body = $recaptcha_verification_result['body'];
        $recaptcha_test = sprintf(
            'ip=%s ua=%s action=%s token_present=%s token_length=%d',
            $remoteip,
            $_SERVER['HTTP_USER_AGENT'] ?? '',
            $body['action'] ?? '',
            isset( $_POST['g-recaptcha-response'] ) ? 'yes' : 'no',
            strlen( $_POST['g-recaptcha-response'] ?? '' ),
        );
        $recaptcha_result = sprintf(
            'rc=%s score=%s errors=%s',
            $rc,
            $body['score'] ?? '',
        ...
        if (
            ...
            error_log(
                "reCAPTCHA missing input: $recaptcha_test $recaptcha_result"
            );
            wp_die( '', '', [ 'response' => 403 ] );
        } elseif ( $body['action'] !== 'subscribe' ) {
            error_log(
                "reCAPTCHA wrong action: $recaptcha_test $recaptcha_result"
            );
            wp_die( '', '', [ 'response' => 403 ] );
        } elseif ( $body['score'] < 0.3 ) {
            error_log(
                "reCAPTCHA suspicious: $recaptcha_test $recaptcha_result"
            );
            wp_die( '', '', [ 'response' => 403 ] );
        } else {
            error_log( "reCAPTCHA report: $recaptcha_test $recaptcha_result" );
        }
    }
    ...
    if ( $errors ) {
        ...
        wp_redirect( "/?$query#fobvSubscribeForm" );
        exit();
